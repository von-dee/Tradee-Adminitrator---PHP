   ...
   public function msg($error_code='0',$msg_text=''){
	...
	return '';
   	...
   	public function getMessages($code){
		global $sql;
		$stmt = $sql->Execute($sql->Prepare("SELECT * FROM {$this->prefix}messages WHERE MESG_STATUS='1' AND MESG_GROUP=".$sql->Param('a')." ORDER BY MESG_DATEADDED DESC LIMIT 4 "),[$code]);
		print $sql->ErrorMsg();
	...
	public function getNotification($code){
		global $sql;
		$stmt = $sql->Execute($sql->Prepare("SELECT * FROM {$this->prefix}notifications WHERE NOTI_STATUS='1' AND NOTI_RECEIVER=".$sql->Param('a')." ORDER BY NOTI_DATE DESC LIMIT 4 "),[$code]);
		print $sql->ErrorMsg();
		if($stmt && $stmt->RecordCount() > 0){
			$obj = $stmt->GetAll();
		}else{
	...
	public function getMenuAccessibility($code){
		global $sql;

		$stmt = $sql->Execute($sql->Prepare("SELECT * FROM {$this->prefix}students_settings WHERE STS_STATUS='1' AND STS_USER_CODE=".$sql->Param('a')." ORDER BY STS_DATEADDED DESC LIMIT 1 "),[$code]);
		print $sql->ErrorMsg();
		if($stmt && $stmt->RecordCount() > 0){
