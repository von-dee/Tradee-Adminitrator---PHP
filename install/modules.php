...
/**
 * dataLayerdata :- The Initial Content of a dataLayer Class used for 
 * accessing database information and normal CRUD
 * You can CHOOSE to white your own data access Logic but this is to jump start the module
 */
$datalayerdata='<?php
class dataLayer extends Engine{
    private $model;
    public  $session,$sql,$keyColumn;
    
function __construct(string $model,string $keyColumn= null){
    global $sql,$session;
    $this->model= $model; 
    $this->sql=$sql;
...
/**
 * Description : function to insert new data
 * @params: $data :- data to insert associative array of key=>value pair
 */
...
private function update(array $data,array $keys):?bool{ 
    if( $this->isKeyValue($data) && $this->isKeyValue($keys) ){
    	return  $this->_update($data, $keys);
    }else{ 
    	return false;
    }
}
...
private function list(array $columns=null,array $search = null):?OS_Pagination{  
    return  $this->_list($columns,$search);
}

private function getSingle( array $where,array $columns =null) {
    if( $this->isKeyValue($where)){
    	return  $this->_getSingle($columns,$where);
    }else{ 
    	return false;
    }
}

private function isKeyValue(array $data){
    return array_values($data)!== $data;
}

private function _getSingle(array $columns =null, array $where){
    if($columns)  {
    ...
    foreach($where as $key=>$value) {
		$set[$i] = $key." = ".$this->sql->Param("b");
		$i++;
    }
    $wherestring = implode(" AND ",$set); 
    $query = "SELECT   $columns FROM ".$this->model." WHERE ".$wherestring;
    $result = $this->sql->Execute($this->sql->Prepare($query),array_values($where)); 
    return $result ;
}

private function _insert(array $data):?bool{
    $columns= implode(",",array_keys($data));
    $params=array_fill(0,count($data),$this->sql->Param("b"));
    $values= implode(",",$params);
    $query = "INSERT INTO ".$this->model." (".$columns.") VALUES (".$values.")"; 
    $result = $this->sql->Execute($this->sql->Prepare($query),array_values($data)); 
    return $result ;
}

private function _update(array $data,array $keys) :?bool{
    $params=array_fill(0,count($data),$this->sql->Param("b"));
    $params= array_combine(array_keys($data),$params);
    $i=0;
    ...
    foreach($keys as $key=>$value) {
		$where[$i] = $key." = ".$this->sql->Param("b");
		$i++;
    } 
    $where=  implode(" AND " ,$where); 
    $query = "UPDATE ".$this->model." SET ".$params." WHERE ".$where."";  
    $result = $this->sql->Execute($this->sql->Prepare($query),array_merge (array_values($data) , array_values($keys))); 
    return $result ;
}

private function _delete(array $where) :?bool{
    $i=0;
    foreach($where as $key=>$value) {
		$exp[$i] = $key." = ".$this->sql->Param("b");
		$i++;
    }
    $Stexp = implode(" AND ",$exp);
    $query = "DELETE FROM $this->model WHERE $Stexp"; 
    $result = $this->sql->Execute($this->sql->Prepare($query),array_values($where) );
    return $result ;
}

private function _list(array $columns =null ,array $search=null):?OS_Pagination{
    global $option,$pg,$limit; 
    if($columns)  {
    	$columns= implode(",",$columns);
    }else{
    	$columns=  "*";
    ...
    if($search){
		$where ="";
		array_walk($search,function($x,$y) use($search,&$where) {   
			$where.= ((count($search) > 1 ) ? $y." LIKE ".$this->sql->Param("a"). ((array_key_last($search)!=$y) ? " OR " : "") :  $y." LIKE ".$this->sql->Param("a"));
		});  
		$query = "SELECT   $columns FROM ".$this->model." WHERE ".$where;
		$input =array_map( function($x){return "%".$x."%";},array_values($search)); 
    }else{ 
    ...
    $lenght = 10;
    $paging = new OS_Pagination($this->sql,$query,$limit,$lenght,"pg=".$pg."&option=".$option, isset($input) ?$input:  []);
    return $paging ;
}

public function __call($name, $arguments){     
	global $microtime,$pg;   
	if($this->validateFormToken($pg)){
		$postkey=  ($name="list") ? true : $this->validatePostForm($microtime);   
		if($postkey){
